<?php
  ...
  class SeatController extends Controller {
     ...
     */
     public function export() {
        ...
        if($res && $params) {
            ...
            $data['status'] = 1;
            //写入操作日志
            write_log(L('DATA_EXPORT')."Seat".L('SUCCESS'));
        }else{
            $data['status'] = 0;
            
            //写入操作日志            
            write_log(L('DATA_EXPORT')."Seat".L('FAILURE').":".L('CREATE_EXCEL_FAILURE'));
    ...
    public function import() {
        ...
        $this->seat_model->commit();
        if($map['status'] == 0){
            $map['params'] = serialize($params);
            //写入操作日志            
            write_log(L('DATA_IMPORT')."Seat".L('FAILURE').":".$params['params']['error_reson']);
        }else{   
            //写入操作日志
            write_log(L('DATA_IMPORT')."Seat".L('SUCCESS'));
        }
        if(M('Queue')->save($map)){
            return true;
        }
