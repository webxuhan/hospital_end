...
hello;
    ...
    public function dologin(){
    	...
    	$rules = array(
    			//array(验证字段,验证规则,错误提示,验证条件,附加规则,验证时间)
    			array('username', 'require', L("MOBILE_EMAIL_MUST"), 1 ),
    			array('password','require',L("PASSWORD_MUST"),1),
...
//        $result = $users_model->where($where)->find();
        ...
        $where['mobile']=trim(I('post.username'));
        $password=I('post.password');
        $result = $Staff->where($where)->find();
 //error_log(print_r($_POST,true),3,DATA_DIR.'/logs/aaa.log');
        if(!empty($result)){
                            ...
                            if(sp_compare_password($password, $result['user_pass'])){//本应用已经有这个用户,且密码正确，同步用户
                                ...
                                if($uc_uid2<0){
                                    $uc_register_errors=array(
                                        "-1"=>L("LOGIN_NAME_ILLEGAL"),
                                        "-2"=>L("CONTAIN_ILLEGAL_WORD"),
                                        "-3"=>L("LOGIN_NAME_EXIST"),
                                        "-4"=>L("EMAIL_FORMAT_ERROR"),
                                        "-5"=>L("EMAIL_NOT_REGISTER"),
                                        "-6"=>L("EMAIL_YET_REGISTER"),
                                    );
                                    $this->error(L("SYNCHRO_USER_ERROR").$uc_register_errors[$uc_uid2]);
